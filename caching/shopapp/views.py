...
class UserOrdersExportView(APIView):
    def get(self, request: HttpRequest, *args, **kwargs) -> JsonResponse:
        user_id = self.kwargs.get('user_id')
        self.owner = get_object_or_404(User, pk=user_id)
        cache_key = f'user_{user_id}_orders_export_data'
        user_orders_data = cache.get(cache_key)
        if user_orders_data is None:
            user_orders = Order.objects.select_related('user').filter(user=self.owner).order_by('pk').all()
